            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form action="{{ route('post.update', $post->id) }}" method="post" class="mt-6 space-y-6" >
                        ...
                        <div>
                            <x-input-label for="posted" :value="__('Publicat')" />
                            <select id="posted" name="posted" class="mt-1 block w-full" >
                                <option value="yes" {{ $post->posted == 'yes' ? 'selected' : '' }}>si</option>
                                <option value="not" {{ $post->posted == 'not' ? 'selected' : '' }}>no</option>
                            </select>
                            <x-input-error :messages="$errors->get('posted')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="category_id" :value="__('Categories')" />
                            <select id="category_id" name="category_id" value="{{ $post->category_id }}" class="mt-1 block w-full"  >
                        ...
                        <div>
                            ...
                            <textarea name="content" col="3" class="form-control" id="editor" rows="3" >{{ $post->content }}</textarea>
